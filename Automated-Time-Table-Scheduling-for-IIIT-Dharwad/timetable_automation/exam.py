...
ROOM_SORT_MODE = "small-first"   # "small-first" or "big-first"
USE_HALLS_LAST = True           # prefer normal rooms; use halls only if needed
...
def collapse_to_broad_group(source_file: str) -> str:
    ...
    return f"{dept}-{sem}" if sem else dept

# --- Domain objects ---
class Course:
    def __init__(self, row, group_name):
        self.group = group_name
        self.code = str(row.get("Course_Code", "")).strip()
...
class ExamScheduler:
    def __init__(self, rooms_file, faculty_file, merged_courses_file="final_merged_courses.csv", start_date=DEFAULT_START_DATE):
        ...
        self.merged_courses_file = merged_courses_file

        self.start_date = datetime.strptime(start_date, "%Y-%m-%d").date()
        # invigilators list: prefer "Name" header if present else first column
        if "Name" in self.invig_df.columns:
            inv_col = "Name"
        else:
            inv_col = self.invig_df.columns[0]
        self.invigilators = sorted([str(n).strip() for n in self.invig_df[inv_col] if str(n).strip()])

        # processed structures
        self.rooms = self._load_rooms()
        self.room_by_id = {r["Room_ID"]: r for r in self.rooms}

        # load courses from single merged file and build groups
        self.courses = self._load_courses()   # dict: group_name -> [Course,...]
    ...
    def _load_rooms(self):
        ...
        for _, r in self.rooms_df.iterrows():
            ...
            cap_raw = str(r.get("Capacity", "0")).strip()
            try:
                cap = int(cap_raw)
            except:
                cap = 0
